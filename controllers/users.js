    ...
    try {
        ...
        if (match) {
            if (user.Deleted === true) {
                ...
                return res.status(200).send({ status: 'success', token: `bearer ${token}`, role: user.User_Category, id: user._id, student_id: user.student_id, email: user.Email })
            }
    ...
    try {
        ...
        if (!validation) {
            ...
            try {
                ...
                const html = fs.readFileSync(path.resolve(__dirname, 'ResetPasswordTemplate.html'), 'utf8').toString()
                    .replace(/\$\{token\}/g, `https://eza-plus.herokuapp.com/resetPassword/${token}`)
